...
incarceration_fixed <- incarceration %>%
  ...
  mutate(black_incarceration_pop = black_jail_pop + black_prison_pop,
         ...
         poc_incarceration_pop = black_incarceration_pop + aapi_incarceration_pop + latinx_incarceration_pop + native_incarceration_pop,
         total_incarceration_pop = black_incarceration_pop + aapi_incarceration_pop + latinx_incarceration_pop + native_incarceration_pop +white_incarceration_pop,
         poc_pop_15to64 = aapi_pop_15to64 + black_pop_15to64 + latinx_pop_15to64 + native_pop_15to64
         )%>%
  select(year,
         state, 
         county_name,
...
current_state_highest <- incarceration_fixed %>%
  ...
  summarize(black_pop_ratio = sum(black_pop_ratio),
            ...
            total_ratio = sum(black_pop_ratio, aapi_pop_ratio, latinx_pop_ratio, native_pop_ratio, white_pop_ratio))%>%
...
## 6) Which county in Washington State has the largest incarceration population currently? ----

# create a dataframe that shows the current total incarceration population in total in each 
# Washington County. 
...
wa_counties_2 <- incarceration_fixed %>%
  ...
  summarize(poc_incarceration_pop = sum(poc_incarceration_pop),
            white_incarceration_pop = sum(white_incarceration_pop))%>%
  mutate(incarceration_total= sum(poc_incarceration_pop, white_incarceration_pop))%>%
  pivot_longer(poc_incarceration_pop:white_incarceration_pop, names_to = "race_incarceration", values_to = "incarceration_pop")%>%
...
pie_chart <- ggplot(wa_counties_2, aes(x="", y=ratio, fill = race_incarceration))+
  ...
  labs(title = "POC VS White WA Most Recent Incarceration Demographic Pie Chart", subtitle = "Shows the ratio of people incarcerated in WA over total people incarcerated in WA") +
  coord_polar(theta = "y")
pie_chart #chart 2
...
map_1 <- plot_usmap(
  ...
  scale_fill_continuous(low = "white", high = "red", name = "POC Ratio", label = scales::comma) + 
