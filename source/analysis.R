...
# summary ----
## 1) Which race has the most incarceration population
...
current_highest_incarceration <- incarceration_fixed %>%
  ...
  filter(pop_total == max(pop_total)) %>%
  pull(race_incarceration)
current_highest_incarceration #black_incarceration_pop

## 3) Which race is currently facing the lowest amount of incarceration?

current_lowest_incarceration <- incarceration_fixed %>%
  filter(year == max(year)) %>%
  summarise(black_incarceration_pop = sum(black_incarceration_pop),
            aapi_incarceration_pop = sum(aapi_incarceration_pop),
            latinx_incarceration_pop = sum(latinx_incarceration_pop),
  ...
  pull(race_incarceration)
current_lowest_incarceration #aapi_incarceration_pop
...
## 5) What group in `current_highest_state` is facing a higher amount of
## incarceration?

current_highest_group <- incarceration_fixed %>%
  filter(year == max(year)) %>%
  group_by(state) %>%
  ...
  pull(demographic)
current_highest_group #poc_pop_ratio
...
## Chart 2: Comparing POC incarceration population to White Persons
...
pie_chart <- ggplot(wa_counties_2,
                    aes(x = "",
                        y = ratio,
                        fill = race_incarceration)) +
  geom_col(color = "black") +
  geom_text(aes(label = race_incarceration),
            position = position_stack(vjust = 0.5)) +
  labs(title =
         "POC VS White WA Most Recent Incarceration Demographic Pie Chart",
       subtitle =
         "Ratio of people incarcerated in WA over total people incarcerated") +
  coord_polar(theta = "y")
pie_chart #chart 2

## Chart 3: map of the ratio of poc incarceration pop to poc total pop
#in WA state
map_white <- incarceration_fixed %>%
  ...
  summarize(white_incarceration_pop = sum(white_incarceration_pop),
            ...
            total_incarceration_pop = white_incarceration_pop +
              poc_incarceration_pop,
...
map_1 <- plot_usmap(
  ...
  scale_fill_continuous(low = "white",
                        ...
                        name = "POC Ratio",
                        label = scales::comma) +
  labs(title = "White Incarceration Ratio in Each State")
map_1
...
map_2 <- plot_usmap(
  ...
  scale_fill_continuous(low = "white",
                        ...
                        name = "POC Ratio",
                        label = scales::comma) +
  labs(title = "Poc Incarceration Ratio in Each State")
map_2